...
public class View {
    ...
    public View() {
        ...
        int id_ang = angajatService.getID();
        int id_cli = clientService.getID();
        if (id_ang > id_cli)
            Persoana.setNrPersoane(id_ang);
        else
            Persoana.setNrPersoane(id_cli);
        Produs.setNrProduse(produseService.getID());
        Restaurant.setNrRestaurante(restauranteService.getID());
        Rezervare.setNrRezervari(rezervariService.getID());
    }
        ...
        while (true) {
            ...
            if (rol.equals("nelogat")) {
                if (command == 1) {// register client
                    ...
                    String[] rez = clientService.loginClient(nume, parola);
                    rol = rez[0];
                    if (!rol.equals("nelogat")) {
                        userID = Integer.parseInt(rez[1]);
...
//                        }
                    ...
                    String[] rez = angajatService.loginAngajat(nume, parola);
                    rol = rez[0];
                    if (!rol.equals("nelogat")) {
                        userID = Integer.parseInt(rez[1]);
